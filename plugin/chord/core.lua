...
local function command_from_entry(source, entry, opts, table_name)
...
---@return Chord.Command|nil
local function registered_command(spec)
  if type(spec) ~= "table" then
    log:error("cannot register command: expected table, got %s", type(spec))
    return nil
  end
  ...
  if action == nil then
    log:error "cannot register command without action"
    return nil
  end
  if not label or label == "" then
    log:error "cannot register command without label"
    return nil
  end
  ...
  M._command_seq = M._command_seq + 1

  local cmd = {
    id = spec.id or ("registered:" .. M._command_seq),
    label = tostring(label),
    action = action,
    source = "registered",
  }
...
---@param opts table
---@param table_name? string
local function collect_entries(commands, seen, source, entries, opts, table_name)
  for _, entry in ipairs(entries or {}) do
    add_command(commands, seen, opts, command_from_entry(source, entry, opts, table_name))
  end
end
...
local function key_table_names(config_table, opts)
  local names = {}
  ...
  if opts.include_key_tables and type(M._defs) == "table" then
    for name in pairs(M._defs) do
      names[name] = true
    end
  end
...
---@param commands Chord.Command[]
---@return table[]
local function command_choices(commands)
  local choices = {}
...
function command.register(spec)
  local cmd = registered_command(spec)
  if cmd then
    M._registered_commands[#M._registered_commands + 1] = cmd
  end
  return cmd
end
...
function command.collect(config_table, opts)
  ...
  if options.include_registered then
    for _, cmd in ipairs(M._registered_commands or {}) do
      add_command(commands, seen, options, shallow_copy(cmd))
    end
  end

  if options.include_keys then
    collect_entries(commands, seen, "keys", config_table.keys or {}, options)
  end

  for _, name in ipairs(sorted_names(key_table_names(config_table, options))) do
    collect_entries(commands, seen, "key_table", resolve_entries(config_table, name), options, name)
  end

  if options.include_defaults and wezterm.gui and type(wezterm.gui.default_keys) == "function" then
    collect_entries(commands, seen, "default", wezterm.gui.default_keys() or {}, options)
  end
  ...
  return wezterm.action_callback(function(window, pane)
    ...
    window:perform_action(
      wezterm.action.InputSelector {
        title = options.title,
        choices = command_choices(commands),
        fuzzy = options.fuzzy,
...
function command.apply(config_table, opts)
  ...
  config_table.keys = config_table.keys or {}
  local entry = M.key(options.key, action, options.desc)
  if entry then
